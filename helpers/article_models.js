...
module.exports = {
    ...
    return new Promise((resolve, reject) => {
      if (!query || query == "") {
        ...
        let queryResult = db
          .get()
          .collection(collection.ARTICLE_COLLECTION)
          .find({ topic: { $regex: query, $options: "i" } })
          .toArray();
        resolve(queryResult);
      }
    });
  ...
  getRecentArticles: () => {
    return new Promise(async (resolve, reject) => {
      let articleDb = await db
        ...
        .limit(4)
        .toArray();
      let aggregatedArticles = await db
        .get()
        .collection(collection.ARTICLE_COLLECTION)
        .aggregate([
          {
  ...
  getArticlesByCategory: (categoryType, noOfArticles) => {
    return new Promise(async (resolve, reject) => {
      let categoryElement = await db
        .get()
        .collection(collection.CATEGORY_COLLECTION)
        .findOne({ trimmed: categoryType });
        ...
        .updateOne(
          { _id: ObjectId(article_id) },
          {
            $set: {
              ...
              category:
                articleDetails.category && ObjectId(articleDetails.category),
  ...
  deleteArticle: (articleId) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collection.ARTICLE_COLLECTION)
        .deleteOne({ _id: ObjectId(articleId) })
        .then((result) => {
          resolve(result);
        });
    });
