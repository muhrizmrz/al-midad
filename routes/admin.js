...
router.get('/settings', authorizeAdmin, async (req, res, next) => {
  try {
    ...
    res.render('admin/settings', { category1, category2, categories, csrfToken1: req.csrfToken(),csrfToken2: req.csrfToken(),csrfToken3:req.csrfToken() });
  } catch (error) {
    next(error);
  }
})
...
router.get('/article/edit/:id', authorizeAdmin, async (req, res, next) => {
  try {
    var id = req.params.id
    let articleToBeEdit = await article_models.getArticleById(id).then(result => result);
